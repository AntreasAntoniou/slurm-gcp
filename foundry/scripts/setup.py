...
def create_users():
    ...
    util.run(
        "useradd -m -c Slurmrestd -d /var/lib/slurmrestd -r slurmrestd -u 982 -g 982"
    )
...
def setup_modules():
    ...
    if modulepaths.exists():
        paths = [
            path
            for path in modulepaths.read_text().splitlines()
...
def install_lustre():
    ...
    if cfg.os_name in ("centos7", "centos8", "rhel7", "rhel8"):
        ...
        util.run(f"wget -nv -r -l1 -np -nd -A '{rpmlist}' '{rpm_url}' -P {lustre_tmp}")
        util.run(f"find {lustre_tmp} -name '*.rpm' -execdir rpm -ivh {{}} ';'")
...
def install_cuda():
    ...
    nvidia_url = (
        f"https://us.download.nvidia.com/tesla/{nvidia_version}/{nvidia_run.name}"
    )
    ...
    util.run(
        f"bash {cuda_run} --silent --toolkit --samples --samplespath={cuda_samples_dir}"
    )
...
def install_libjwt():
    ...
    util.run(
        "git clone --single-branch --depth 1 -b {0} {1} {2}".format(
            cfg.libjwt_version, GIT_URL, src_path
        )
    )
    ...
    with cd(build_path):
        util.run(
            f"{src_path}/configure --prefix={JWT_PREFIX} --sysconfdir={JWT_PREFIX}/etc",
            stdout=DEVNULL,
        )
...
"""
    ...
    util.run(
        f"{cfg.pacman} install -y {' '.join(packages)}",
        shell=True,
...
def install_slurm():
    ...
    with cd(src_path):
        ...
        if cfg.slurm_version.startswith("b:"):
            ...
            util.run(
                "git clone --single-branch --depth 1 -b {0} {1} {0}".format(
                    use_version, GIT_URL
                )
            )
        else:
            ...
            use_version = util.run(
                f"tar -xvjf {tarfile}", check=True, get_stdout=True
    ...
    with cd(build_dir):
        util.run(
            f"{src_path}/configure --prefix={dirs.install} --sysconfdir={slurmdirs.etc} --with-jwt={dirs.install}"
        )
        util.run("make -j install", stdout=DEVNULL)
...
def setup_bash_profile():
    ...
    Path("/etc/profile.d/slurm.sh").write_text(
        f"""
S_PATH={dirs.install}
...
def install_ompi():
    ...
    ompi_src.mkdirp()
    util.run(
        f"git clone --single-branch --depth 1 -b {cfg.ompi_version} {ompi_git} {ompi_src}"
    )
    with cd(ompi_src):
