...
def GenerateComputeVM(context, create_disks_separately=True):
    ...
    if GUEST_ACCELERATORS in prop:
        ...
        resource[0]["properties"].update({GUEST_ACCELERATORS: prop[GUEST_ACCELERATORS]})
        ...
        if ip_value != "EPHEMERAL" and ip_value != "NONE" and not is_static_ip:
            raise common.Error(
                (
                    ...
                    "are: a valid IP Address, EPHEMERAL or NONE."
                )
                % ip_value
            )
...
def SetMetadataDefaults(metadata):
    ...
    if not [True for x in items if x.get("key", None) == "google-monitoring-enable"]:
        items.append({"key": "google-monitoring-enable", "value": "0"})
    if not [True for x in items if x.get("key", None) == "google-logging-enable"]:
        items.append({"key": "google-logging-enable", "value": "0"})
        ...
        local_ssd_disks.append(
            {
                ...
                "autoDelete": True,
                "initializeParams": {
                    "diskType": common.LocalComputeLink(
    ...
    resource = [
        {
            ...
            "properties": {
                ...
                "dnsIntegration": {"networks": [network]},
            },
        }
    ]
    return resource
        ...
        outputs.append(
            {
                "name": "ip",
                "value": (
                    "$(ref.%s.networkInterfaces[0].accessConfigs[0].natIP)"
                    % vm_res["name"]
                ),
            }
        )
