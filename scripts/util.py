...
def config_root_logger(level="DEBUG", util_level=None, stdout=True, logfile=None):
    if not util_level:
        util_level = level
...
def run(
    ...
    **kwargs,
    ...
    ret = subprocess.run(
        args, shell=shell, universal_newlines=universal_newlines, **kwargs
    )
    if wait:
        time.sleep(wait)
    return ret
...
def get_regional_instances(compute, project, def_list):
    ...
    region_filter = " OR ".join(
        f"(name={pid}-*)" for pid, d in def_list.items() if d.regional_capacity
    )
    if region_filter:
        page_token = ""
        while True:
...
def to_hostlist(scontrol, nodelist=[]):
    ...
    hostlist = run(
        f"{scontrol} show hostlist {tmp_file.name}", shell=True, get_stdout=True
    ).stdout.rstrip()
